...
def registrar_clientes():
    try:
        ...
        sql="""INSERT INTO CLIENTES(Nombre, apellidos, empresa, puesto, CP, provincia, telefono, fechaNacimiento) 
        VALUES ('{0}','{1}','{2}','{3}',{4},'{5}',{6},'{7}')""".format(request.json['Nombre'],request.json['apellidos'],request.json['empresa'],request.json['puesto'],request.json['CP'],request.json['provincia'],request.json['telefono'],request.json['fechaNacimiento'])
        cursor.execute(sql)
        conexion.connection.commit() #Confirma la accion de insercion
        return jsonify({'Mensaje':"CLIENTE REGISTRADO"})
    except Exception as ex:
        return jsonify({'Mensaje':"ERROR BACKEND"})
